...
# Standard output format: <number>
answer_3() {
	awk ' BEGIN {FS="," ; sum=0} ; {if ($7 ~/The Beatles/) {sum++;}} ; END {print sum}' million_songs_metadata.csv 
}
...
answer_4() {
	# inspired from [0]
    awk 'BEGIN{FS=","} ; { sum += $8; n++ }; END {print sum / n;}' million_songs_metadata.csv
}
...
answer_5() {
	# inspired form [1],[2]
    awk 'BEGIN {FS = ","; OFS=","} FNR==NR{a[$1] = $2 FS $3 FS $4 FS $5 FS $6 FS $7 FS $8 FS $9 FS $10 FS $11;next} { print $0,a[$1]}' million_songs_metadata.csv million_songs_sales_data.csv > million_songs_metadata_and_sales.csv
}
...
answer_6() {
	var=$(awk 'BEGIN{FS=","};{a[$7]+=$3} END{for(i in a) print i, a[i]}' million_songs_metadata_and_sales.csv | sort -k 2 -n -r | head -n 1 | cut -c1-18)
    grep $var million_songs_metadata_and_sales.csv | cut -d "," -f 9 | head -n 1
}
